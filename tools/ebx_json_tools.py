...
def encodeScreenLayout2JSON(pseudo_json:dict, target_filename:str = "blank.json"):    
    ...
    with open(target_filename, 'r', encoding='utf-8') as f:
        ...
        if screen_name not in _screen_names:
            raise Exception(f"screen name `{screen_name}` not found in the project `{target_filename}`, please check")
...
"""
def decodeScreenLayoutFromJSON(screen_name:str, filename:str = "blank.json"):
    # open file
    with open(filename, 'r', encoding='utf-8') as f:
...
def upsertObjects(widget_list:list, screen_name:str, target_filename:str = "blank.json"):
    ...
    with open(target_filename, 'r', encoding='utf-8') as f:
        ...
        # 檢查screen name
        if screen_name not in _screen_names:
            out = f"[create object failed] screen name `{screen_name}` not found, please check"
            return out
